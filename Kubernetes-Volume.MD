...
##### *```NOTE```: node1.txt, which was created at directory location /volume/hostpath/ of Node#1 can be viewed from inside of the Pod that gets on Node#1. Similarly, node2.txt which was created at directory location /volume/hostpath/ of Node#2 can be viewed from inside of the Pod that gets on Node#2.*
