...
##### *```NOTE```: node1.txt which was created in /volume/hostpath/ of Node#1 can be viewed from the Pod(/mnt-vol) which lands on Node#1. Similarly, node2.txt which was created in /volume/hostpath/ of Node#2 can be viewed from the Pod(/mnt-vol) that lands on Node#2.*
<hr>
