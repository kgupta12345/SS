...
##### *```NOTE```: node1.txt, which was created under /volume/hostpath/ of Node#1 is visible by Pod (by navigating at /mnt-vol). As the Pod is running on Node#1. Similarly, node2.txt which was created under /volume/hostpath/ of Node#2 is visible by Pod (by navigating at /mnt-vol) as the Pod is running on Node#2.*
